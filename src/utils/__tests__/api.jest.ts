...
function getMockResponse(data: any, ok: boolean = true) {
...
describe("Test api", () => {
    ...
    test("Test resetPasswordApi fail", async () => {
        ...
        const mockUserData = { password: "test", token: "testtoken" };
        await API.resetPasswordApi(mockUserData).catch((err) => expect(err).toEqual(mockError));
    });
    ...
    test("Test forgotPasswordApi fail", async () => {
        global.fetch = jest.fn(() => getMockResponse(mockError)) as any;
        await API.forgotPasswordApi({ email: "test@mail.com" }).catch((err) => expect(err).toEqual(mockError));
    });
    ...
    test("Test loginUserApi fail", async () => {
        const mockUserData = { email: "test@mail.com", password: "test" };
        global.fetch = jest.fn(() => getMockResponse(mockError)) as any;
        await API.loginUserApi(mockUserData).catch((err) => expect(err).toEqual(mockError));
    });
    ...
    test("Test registerUserApi fail", async () => {
        ...
        global.fetch = jest.fn(() => getMockResponse(mockError)) as any;
        await API.registerUserApi(mockUserData).catch((err) => expect(err).toEqual(mockError));
    });
    ...
    test("Test orderBurgerApi fail", async () => {
        global.fetch = jest.fn(() => getMockResponse(mockError)) as any;
        const burger = ["testid1", "testid2", "testid1"];
        await API.orderBurgerApi(burger).catch((err) => expect(err).toEqual(mockError));
    });
    ...
    test("Test getIngredientsApi fail", async () => {
        global.fetch = jest.fn(() => getMockResponse(mockError)) as any;
        await API.getIngredientsApi().catch((err) => expect(err).toEqual(mockError));
    });
